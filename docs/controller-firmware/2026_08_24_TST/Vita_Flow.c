...
enum{STD_BY,BLEEDING_WSYSTEM,TEST_PERIFERAL,DISPENSER_TEST,WATERFILLING_TEST,SWATERFILLING_TEST}clean_mix_state;  
...
eeprom unsigned char skoef_w100;     // Koef_SWater*100=skoef_w100  (1.2*100=120)
eeprom float swkoeff_time;           // коэффициент пересчета времени проистекания газированной воды.


float timer_prepare_bevarege_pf,data_water_tempf;
float  koef_w100fl, Koef_Water;      // Koef_Water*Water=pulses water counter ()
float  skoef_w100fl, Koef_SWater;    // Koef_SWater*Water=pulses soda water counter ()
...
unsigned int data_water_temp;
float water_cnt_temp;
unsigned char Error; 
...
//========================================================
flash unsigned char ver_year=26,ver_month=8,ver_day=24,sub_ver=1;
//========================================================
//=========================================================
...
//------------------------------------------------------------------------
 ...
 //----------------------------------------------------
  // soda water koefficient
  skoef_w100fl=skoef_w100;      // char to float
  
   if((skoef_w100fl==0)||(skoef_w100fl>=800)) {skoef_w100=300; skoef_w100fl=koef_w100; Koef_SWater=3.0;}    // default value koeff soda water 3.0
      else
        Koef_SWater=(float)(skoef_w100fl/100);       
                  ...
                  case 0x50:  // Command =0x50  // PC INCOMMING COMMAND:  Proteine COCTALE !!!!
                                            ...
                                            if(settings_cmam==9)  // go to Test dispenser
                                             ...
                                             if(settings_cmam==10)  // go to Test Water fillings to glass
                                               { 
                                                 ...
                                                 Klap_Data(1);  // water valve: WATER FILTERED VALVE
                                               ...
                                               if(settings_cmam==11)  // go to Test SODA Water fillings to glass
                                                 { 
                                                  Start_Of_Prepare_Beverage();   // Message to PC 
                                                  data_water_temp=(unsigned int)(Data_Water1*10);   // Data_Water1=Ml/10 (100ml -> 10)
                                                  water_counter=(unsigned int)(data_water_temp*Koef_SWater);  // CALCULATE WATER COUNTER VALUE (PULSES) 
                                                  water_counter_data=0;  // reset water counter 
                                                  timer_fault=0;
                                                  timer_prepare_bevarege_p=0;  // reset prepereing timer 
                                                 
                                                  Klap_Data(3);  // water valve: WATER CARB VALVE
                                                  Water_Pump_Valve(1);   // Water pump on
                                                  Peristalic_Pump_On_Off(0);   
                                                     
                                                  clean_mix_state=SWATERFILLING_TEST;
                                                  mode=MIXER_CLEAN;
                                               }
                   ...
                   case 0xB8:  // Reset water counters
                    ...
                    case 0xBB:  // Command =0xBB  
                                   ...
                                   koef_w100=buff_rx[1];  // water koefficient
                                   
                                   skoef_w100=buff_rx[3]; // soda water koefficient
                                  //----------------------------------------
                                    // water koefficient 
                                   koef_w100fl=koef_w100;      // char to float
                            ...
                            case 0xBC:  // Command =0xBC Read Water/SWater koeff
             ...
             if(clean_mix_state==WATERFILLING_TEST) //only water filling to glas test
               {
                ...
                //================================================================ 
                 if(clean_mix_state==SWATERFILLING_TEST) //only soda water filling to glas test
                   {
                    if(water_counter_data>=water_counter)
                      { 
                       timer_prepare_bevarege_pf=timer_prepare_bevarege_p;
                       data_water_tempf=data_water_temp;
                       // calculated koefficient for max time prepareing
                       swkoeff_time=(float)(timer_prepare_bevarege_pf/data_water_tempf);    //data_water_temp in ml;    timer_prepare_bevarege_p in msec 
