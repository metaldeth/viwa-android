...
class TelemetryAckRouterTest {
    ...
    private lateinit var loyaltyWaterStore: LoyaltyWaterOutboxStore
    ...
    fun setUp() {
        ...
        loyaltyWaterStore = LoyaltyWaterOutboxStore(outboxStore)
        router = TelemetryAckRouter(outboxStore)
    }

    @Test
    fun `routes saleId ack to outbox when generation matches`() = runTest {
        outboxStore.enqueueSale(sampleSale("sale-abc"))
        val row = persistence.allRows().single()
        outboxStore.markInFlight(row, sessionGeneration = 1L)
        val payload = buildJsonObject { put("saleId", "sale-abc"); put("ok", true) }
        val envelope =
            MvpWsEnvelopeDto(
                type = "ack",
                messageId = "ack-1",
    ...
    @Test
    fun `rejects stale saleId ack from prior session generation`() = runTest {
        outboxStore.enqueueSale(sampleSale("sale-stale"))
        val row = persistence.allRows().single()
        outboxStore.markInFlight(row, sessionGeneration = 1L)
        val payload = buildJsonObject { put("saleId", "sale-stale"); put("ok", true) }
        val envelope =
            MvpWsEnvelopeDto(
                type = "ack",
    ...
    fun `rejects stale requestUuid ack from prior session generation`() = runTest {
        ...
        loyaltyWaterStore.enqueueWaterUse(clientId = "client-1", requestUuid = requestUuid, volumeMl = 200)
        val row = persistence.allRows().single()
    ...
    @Test
    fun `technician ack with ok only does not invoke loyalty handler`() = runTest {
        val loyaltyHandler = mockk<suspend (String, JsonObject) -> Unit>(relaxed = true)
        val envelope =
            MvpWsEnvelopeDto(
                type = "ack",
                messageId = "ack-tech",
        ...
        val outcome =
            router.routeAck(
                ...
                technicianHandler = null,
            )
        assertEquals(AckRouteOutcome.ORPHAN, outcome)
        coVerify(exactly = 0) { loyaltyHandler(any(), any()) }
    }
